...
switch classifierType
 ...
 case {'randomForest'}
  ...
  [label scores cost] = predict(B, examples);

 case {'deepLearning'}

  nn = models{nClasses+1};

  test_x = zscore( examples );
  %test_y = categorical2binary( test.out, nClasses );
  test_y = nan(size(examples,1), nClasses); % HACK, since we're only using feedforward nn, test_y can be a dummy
